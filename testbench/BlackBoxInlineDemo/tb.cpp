...
#include <iostream>
#include <fstream>
#include "VBlackBoxInlineDemo.h" // 译码器模块类的头文件
using namespace std;
// 单词dump的意思是 转存
...
VBlackBoxInlineDemo *dut;      // 顶层dut对象指针
...
int main(int argc, char **argv)
{
    ...
    // 为对象分配内存空间
    dut = new VBlackBoxInlineDemo; // 这个名字是仿真的top模块所在的verilog文件的名字前加个V
    mTracer = new VerilatedVcdC;

    // tfp初始化工作
    dut->trace(mTracer, 99); // Trace 99 levels of hierarchy
    mTracer->open("./testbench/BlackBoxInlineDemo/tb.vcd"); // 指定输出波形文件的文件名
